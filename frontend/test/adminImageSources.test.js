...
const image = {
  placeholder: "/images/one/placeholder.jpg",
  sources: {
    jpeg: ["thumb", "small", "medium", "large"].map((variant, index) => ({ variant, width: 320 * (index + 1), url: `/images/one/${variant}.jpg` })),
    webp: ["thumb", "small", "medium", "large"].map((variant, index) => ({ variant, width: 320 * (index + 1), url: `/images/one/${variant}.webp` })),
  },
}

test("admin grid selects only thumb WebP and JPEG sources", () => {
  const sources = adminImageSources(image, "thumb")
  assert.equal(sources.webp.url, "/images/one/thumb.webp")
  assert.equal(sources.jpeg.url, "/images/one/thumb.jpg")
  assert.equal(sources.placeholder, "/images/one/placeholder.jpg")
