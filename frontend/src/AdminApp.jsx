    ...
    <main className="min-h-screen bg-neutral-100 p-4 sm:p-8">
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Photo settings</h1>
            <p className="text-sm text-neutral-600">Add photos and update their descriptive details.</p>
          </div>
          <button onClick={logout} className="rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm">Log out</button>
        </header>
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <section className="rounded-lg bg-white p-5 shadow">
            <h2 className="mb-4 text-lg font-semibold">{selected ? "Edit photo" : "Add photo"}</h2>
            <AdminPhotoForm 
              key={selected?.id || "new"} 
              mode={selected ? "edit" : "create"} 
              photo={selected} 
              saving={saving} 
              onSubmit={selected ? saveEdit : saveCreate} />
            {selected && <button className="mt-4 text-sm underline" onClick={() => setSelected(null)}>Add a different photo</button>}</section><aside className="rounded-lg bg-white p-5 shadow"><h2 className="mb-4 text-lg font-semibold">Existing photos</h2><div className="grid grid-cols-2 gap-3 lg:grid-cols-1">{photos.map((photo) => <button key={photo.id} onClick={() => setSelected(photo)} className={`overflow-hidden rounded-md border text-left ${selected?.id === photo.id ? "border-neutral-900" : "border-neutral-200"}`}><img className="h-28 w-full object-cover" src={photo.sources} alt={photo.title || "Photo"} /><span className="block truncate p-2 text-sm">{photo.title || "Untitled photo"}</span></button>)}</div></aside></div></div></main> )
}