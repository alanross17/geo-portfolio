...
export default function GuessPanel({ onGuess, result }) {
  ...
  return (
    <>
      <div
        className={`fixed bottom-20 left-1/2 w-[92vw] max-w-xl -translate-x-1/2 rounded-xl overflow-hidden shadow-lg bg-white/85 backdrop-blur-sm border border-gray-200 h-[40vh] min-h-[240px]
      ...
      >
        <div className="absolute top-2 right-2 z-10 md:hidden">
          <button
            className="rounded bg-white/90 px-3 py-1 text-xs font-medium text-gray-800 shadow"
      ...
      {collapsed && (
        <button
          className="fixed bottom-16 right-4 rounded-full bg-white/95 px-4 py-3 text-xs font-semibold text-gray-800 shadow-lg border border-gray-200 md:hidden"
